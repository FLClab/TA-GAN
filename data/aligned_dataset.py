...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        STED_transform = get_transform(self.opt, transform_params, grayscale=True)

        confocal = confocal_transform(confocal)
        STED = STED_transform(STED)

        return {'confocal': confocal, 'STED': STED, 'image_paths': image_path}

...
class Pix2PixModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.real_A = input["confocal"].to(self.device)
        self.real_B = input["STED"].to(self.device)
    ...
    def generate_STD_maps(self):
        self.visual_names = ['STD_map']
        fake_B = self.netG(self.real_A)
        fake_B_array = np.zeros((fake_B.shape[2],fake_B.shape[3],50))
        for i in range(50):
            fake_B_array[:,:,i] = self.netG(self.real_A).cpu().numpy()
        self.STD_map = np.std(fake_B_array, axis=2)/np.mean(fake_B_array, axis=2)
        self.STD_map = torch.from_numpy(self.STD_map).unsqueeze(0).unsqueeze(0)

    def forward(self):
        #self.generate_STD_maps()

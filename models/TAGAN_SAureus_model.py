...
class TAGANSAureusModel(BaseModel):
    """ This class implements the TA-GAN model for BF to SIM resolution enhancement for the axonal rings dataset.
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['BF', 'SIM', 'fake_SIM', 'seg_SIM', 'seg_fakeSIM', 'seg_GT']
    ...
    def set_input(self, input):
        ...
        """
        self.BF = input['confocal'].to(self.device)
        self.SIM = input['STED'].to(self.device)
    ...
    def forward(self):
        ...
        if not self.isTrain:
            self.fake_SIM = torch.zeros((self.opt.num_gens,1,self.BF.shape[2],self.BF.shape[3]))
            self.seg_fakeSIM = torch.zeros((self.opt.num_gens,1,self.BF.shape[2],self.BF.shape[3]))
            self.seg_SIM = self.netS(self.SIM)

            for i in range(self.opt.num_gens):
                fake_SIM = self.netG(self.BF)
                seg_fakeSIM = self.netS(fake_SIM)
                self.fake_SIM[i,:,:,:] = fake_SIM
                self.seg_fakeSIM[i,:,:,:] = seg_fakeSIM
        else:
            self.fake_SIM = self.netG(self.BF)
            self.seg_SIM = self.netS(self.SIM)
            self.seg_fakeSIM = self.netS(self.fake_SIM)

    def backward_D(self):
        """Calculate GAN loss for the discriminator"""
        # Fake; stop backprop to the generator by detaching fake_B
        fake_BF_SIM = torch.cat((self.BF, self.fake_SIM), 1) 
        pred_fake = self.netD(fake_BF_SIM.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False) * self.opt.lambda_GAN
        # Real
        real_BF_SIM = torch.cat((self.BF, self.SIM), 1)
        pred_real = self.netD(real_BF_SIM)
    ...
    def backward_G(self):
        ...
        fake_BF_SIM = torch.cat((self.BF, self.fake_SIM), 1)
        pred_fake = self.netD(fake_BF_SIM)
        self.loss_G_GAN = self.criterionGAN(pred_fake, True) * self.opt.lambda_GAN

        # Segmentation loss
        self.loss_S_fake = self.criterionSEG(self.seg_fakeSIM, self.seg_GT) * self.opt.lambda_seg # loss between fake B segmentation and GT
    ...
    def backward_S(self):
        if self.isTrain:
            self.loss_S_real = self.criterionSEG(self.seg_SIM, self.seg_GT) * self.opt.lambda_seg # loss between real B segmentation and GT

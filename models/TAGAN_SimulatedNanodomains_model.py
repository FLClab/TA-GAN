...
class TAGANSimulatedNanodomainsModel(BaseModel):
    """ This class implements the TA-GAN model for confocal to STED resolution enhancement for the simulated nanodomains dataset.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='resnet_9blocks', dataset_mode='axons', netS='resnet_6blocks', name='TAGANSimulatedNanodomains_{}'.format(datetime.date.today()))
        if is_train:
            parser.set_defaults(pool_size=0, gan_mode='vanilla', niter=900, niter_decay=100, batch_size=64, preprocess='rotation')
            parser.add_argument('--lambda_GAN', type=float, default=1, help='weight for GAN loss')
            parser.add_argument('--lambda_seg', type=float, default=100, help='weight for seg loss')
        return parser
    ...
    def forward(self):
        ...
        self.seg_STED = self.netS(self.STED)  # S_B(B)
